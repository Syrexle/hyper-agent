...
def test_volatility_sizer_keeps_fixed_notional_and_caps_leverage_at_two():
    candles = [
    ...
    settings = Settings(_env_file=None, fixed_notional_usd=Decimal("10"), max_leverage=Decimal("2"))

    sizing = VolatilitySizer(settings).calculate(candles, price=2.0)

    assert sizing.notional_usd == Decimal("10")
    assert sizing.leverage <= Decimal("2")
